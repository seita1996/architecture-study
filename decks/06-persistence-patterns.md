...
# 第6回: Active Record、Data Mapper、Repository、Unit of Work

永続化周辺のパターンを整理する

<!--
話すこと:
- この回は「第6回: Active Record、Data Mapper、Repository、Unit of Work」を学ぶ時間だと伝える。最初に正解を覚える場ではなく、判断材料を増やす場だと置く。
...
<!--
話すこと:
- このスライドでは「今回の問い」を、第6回: Active Record、Data Mapper、Repository、Unit of Work の理解につながる部品として説明する。
...
- Unit of Work がトランザクション境界の話だとわかる
...
await saveInvoice(paidInvoice)
```

データと永続化処理を同じモデル操作として扱う。
...
<!--
話すこと:
- このスライドでは「Data Mapper の見方」を、第6回: Active Record、Data Mapper、Repository、Unit of Work の理解につながる部品として説明する。
- まず何の問題を扱っているのかを確認し、その後で名前や分類を紹介する。
- 最後に、現在の設計判断にどう関係するかを一言でつなげる。
-->
---
...
await unitOfWork.transaction(async (tx) => {
  ...
  await tx.auditLogs.record(event)
})
```

複数の変更を 1 つのトランザクションとして扱う。
...
<!--
話すこと:
- このスライドでは「論点」を、第6回: Active Record、Data Mapper、Repository、Unit of Work の理解につながる部品として説明する。
- まず何の問題を扱っているのかを確認し、その後で名前や分類を紹介する。
- 最後に、現在の設計判断にどう関係するかを一言でつなげる。
-->
---
...
- テストで外部 I/O を差し替えたい
- 複数テーブルを 1 つの集約として扱う
- 外部 API や Queue と同じ出力ポートとして扱う

<!--
話すこと:
- このスライドでは「Repository が意味を持つ場合」を、第6回: Active Record、Data Mapper、Repository、Unit of Work の理解につながる部品として説明する。
- まず何の問題を扱っているのかを確認し、その後で名前や分類を紹介する。
- 最後に、現在の設計判断にどう関係するかを一言でつなげる。
-->
---
