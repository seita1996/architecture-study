...
| Delivery semantics | at-least-onceを前提とし、Consumerや後続処理を冪等にする |

非同期にする理由は「先進的だから」ではない。
失敗時に再試行でき、画面応答と切り離せるから。
at-least-once は Broker から無限に必ず届くという意味ではない。
保持期限、恒久障害、手動修復も設計する。
...
|---|---|
| 二重登録 | unique job key |
| 複数Workerの同時取得 | atomic claim |
| Worker停止 | lease |
| retry / complete | 状態、attemptCount、nextAttemptAt、最大試行回数 |
...
冪等性識別子:
`notificationRequestId`、`messageId`、`deliveryIntentId`

実現手段:
unique constraint、Inbox / processed-message、配信状態テーブル、ProviderのIdempotency Key
