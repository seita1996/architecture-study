...
Follow-up Decisions:
  Issue #123（例）:
    運用要件、既存Broker、処理量を調査する。
    Outcome: ADR-013でJob QueueかOutbox Relayかを決定する。
    Owner: Invoice team
    Due: 次回計画前
    Status: Open
  ADR-014（Proposed）:
...
- Follow-up Decision は別ADRまたはIssueへリンクする。ADRは判断、Issueは調査や実装作業として分ける。
- Owner、期限、Statusを持たせると放置されにくい。
-->
---
...
全問を必ず聞く必要はない。
初回回答で欠けていた項目に対して、同じ文言を使う。
点数は出現タイミングだけで決めない。
無介入で出ても具体化や判断への利用が不十分なら2点にはしない。
...
- パターン名を思い出せなくても、問題、解決構造、コストを説明できれば設計判断として評価する。
-->
---
