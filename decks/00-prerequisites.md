...
1. 事前診断で5〜10問に答える
2. 不足した項目だけ補足資料を読む
3. 第0回では、全員が詰まった上位2〜3項目だけ講師が説明する

<!--
話すこと:
- 知らない言葉があっても問題ない。ただし、本編で何度も出るため、ここで最低限の意味を押さえる。
- 全員に同じ基礎講義をするのではなく、診断で不足した項目に絞る。知っている人には冗長で、知らない人には速すぎる状態を避ける。
-->
---
...
この教材でいう同期・非同期は、主にシステム間・処理間で、呼び出し元が最終処理完了を待つかという意味。
JavaScript の `async` キーワードとは別の軸。
...
| 用語 | 最低限の意味 |
|---|---|
| unit test | 小さな振る舞い単位を高速に検証する。必要に応じて test double を使う |
