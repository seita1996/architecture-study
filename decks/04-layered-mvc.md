...
<!--
話すこと:
- この回は「第4回: 論理構造とコード配置」を学ぶ時間だと伝える。最初に正解を覚える場ではなく、判断材料を増やす場だと置く。
...
<!--
話すこと:
- このスライドでは「今回の問い」を、第4回: 論理構造とコード配置 の理解につながる部品として説明する。
...
- Package by Layer は Layered Architecture の配置例である
...
Layer は単なる置き場所ではなく、どの責務がどの責務へ依存してよいかという規則も含む。
...
- 理解しやすい
- 技術責務が明確
- Controller / Service / Repository の素直な構成は小規模 CRUD と相性がよい
- フレームワークの標準構成と合いやすい

<!--
話すこと:
- このスライドでは「長所」を、第4回: 論理構造とコード配置 の理解につながる部品として説明する。
- まず何の問題を扱っているのかを確認し、その後で名前や分類を紹介する。
- 最後に、現在の設計判断にどう関係するかを一言でつなげる。
-->
---
...
小さな CRUD では、Layered + Package by Layer + Transaction Script の素直な組み合わせがかなり強い。

Layered Architecture 自体は、大規模なシステムでも使われる。
ここで小規模 CRUD と相性がよいのは、単純な Controller / Service / Repository 構成のこと。
...
| Model | Domain Model、Read Model、DTO など文脈で意味が変わる |
...
- Package by Layer だと、1 つの機能が複数ディレクトリに分散しやすい
- 層の責務が曖昧だと、Service が肥大化しやすい
- 依存規則が曖昧だと、レイヤーを飛び越えた依存が発生しやすい
- 機能単位の変更範囲が見えにくいことがある

これは Layered Architecture の必然ではなく、配置や運用規則で起きやすい問題。

<!--
話すこと:
- このスライドでは「起きやすい失敗」を、第4回: 論理構造とコード配置 の理解につながる部品として説明する。
- まず何の問題を扱っているのかを確認し、その後で名前や分類を紹介する。
- 最後に、現在の設計判断にどう関係するかを一言でつなげる。
-->
---
...
同じ Layered Architecture でも、機能単位に置くことはできる。
...
Dependency Inversion を適用すると、次のようにも見られる。
...
Package by Layer では責務が分かりやすい一方、1つの機能変更が複数ディレクトリへ広がりやすい。
