...
依存が消えるのではなく、位置への依存が、より明示的な名前への依存へ移る。

<!--
話すこと:
- Connascence は補助語彙として扱う。後続回で積極的に使わないなら、ここで深掘りしない。
- object引数にしても依存は消えない。順序という暗黙の依存を、名前という明示的で弱い依存へ移す。
-->
---
...
| 型安全性 | 契約に `paid`、請求書に `active` を渡せる型になってしまう |
