...
type ValidateStatus = (
  status: string,
  type: "invoice" | "contract" | "subscription",
) => boolean

export const validateStatus: ValidateStatus = (status, type) => {
  if (type === "invoice") return ["draft", "issued", "paid"].includes(status)
  if (type === "contract") return ["active", "expired"].includes(status)
  return ["trial", "active", "cancelled"].includes(status)
}
```

重複は減ったが、請求書、契約、サブスクリプションの変更理由が 1 つの関数に集まった。
...
次のコードを見て、`validateStatus` を共通関数にしてよいか考える。

```ts
type InvoiceStatus = "draft" | "issued" | "paid" | "cancelled"
type ContractStatus = "draft" | "active" | "cancelled"

type StatusTarget = "invoice" | "contract"

const validateStatus = (status: string, target: StatusTarget): boolean => {
  if (target === "invoice") {
    return ["draft", "issued", "paid", "cancelled"].includes(status)
  }

  return ["draft", "active", "cancelled"].includes(status)
}
```
