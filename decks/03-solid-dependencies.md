---
theme: default
title: "第3回: SOLIDと依存関係"
---

# 第3回: SOLID と依存関係

SOLID を暗記ではなく、依存関係を扱うための判断軸として使う

<!--
話すこと:
- この回は「第3回: SOLID と依存関係」を学ぶ時間だと伝える。最初に正解を覚える場ではなく、判断材料を増やす場だと置く。
...
<!--
話すこと:
- このスライドでは「今回の問い」を、第3回: SOLID と依存関係 の理解につながる部品として説明する。
...
|---|---|
| SRP | 変更理由を 1 つに近づける |
| OCP | 既存コードを壊さず追加しやすくする |
| LSP | 同じ型なら同じ期待で使えるようにする |
| ISP | 必要なものだけに依存する |
| DIP | 不安定な詳細ではなく安定した抽象へ依存する |
...
```ts
type CreateInvoice = (command: IssueInvoiceCommand) => Promise<IssueInvoiceResult>
...
Prisma の API、DB モデル、トランザクション方針がユースケースに見える。
...
const createInvoice =
  ...
  async (command) => {
    const issued = await invoices.findIssuedByContract(command.contractId)
    if (issued) return { type: "already_issued" }

    const invoice = issueInvoice(command)
...
実装 A は 1 円以上を受け付ける。実装 B は 1,000 円未満で独自エラーを返す。
