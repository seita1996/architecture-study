...
- 同じ Invoice Aggregate を複数 Slice に複製しない。共有 Domain として置くか、機能群モジュールに寄せる
- Prisma Adapter は、単純 CRUD なら共通 Query としてよい。複雑な Slice だけ専用 Adapter にする
- 認可、ログ、トランザクションは Composition Root や middleware で共有し、業務判断だけ Slice に置く
...
|---|---|---|
| 初期理解 | 容易 | 慣れが必要 |
| 機能変更 | 複数層に分散 | 近い場所に集まる |
| 技術責務 | 見えやすい | Slice 内に閉じる |
| 重複 | 少なくしやすい | 許容判断が必要 |
| 境界 | 技術責務の境界 | ユースケースの境界 |
...
features/resend-invoice/use-case.ts
features/resend-invoice/ports.ts
...
次回は、Slice 内部の分岐や状態遷移を説明するデザインパターンを見る。
