...
# 第1回: アーキテクチャとデザインパターンの地図

設計の言葉をそろえ、以降の比較で迷子にならないための回
...
- これは「アーキテクチャ」の話
- これは「パターン」の話
- これは「ライブラリやフレームワーク」の話

粒度を分けられると、議論がかなり楽になる。
...
const issueInvoice =
  ...
  async (command) => {
    const invoice = await saveInvoice(command)
