...
<!--
話すこと:
- 強調: Clean Architecture は円の図ではなく依存規則。
- 典型的な誤答: 外部依存はすべてPortにすればよい。
- 問い返す: そのPortはどの失敗契約や変更理由を隔離するのか。
-->
---
...
Output Port がアプリケーションから見える失敗契約を定義し、Output Adapter が Prisma 固有のエラーや更新競合をその型へ変換する。
Application Service は、その結果をユースケースの結果へ変換する。

<!--
話すこと:
- 答え合わせでは唯一の正解としてではなく、判断の筋道として説明する。
- どの情報を見てそう判断したのかを明示し、参加者が自分の考えと照合できるようにする。
- 失敗契約を完全実装する回ではないが、Port は差し替えだけでなく失敗表現の変換点でもあると回収する。
-->
---
