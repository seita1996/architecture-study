...
演習は長いグループワークにはしません。ただし、判断は必ず外部化します。各自がスライド上の実例やコード断片を読み、次の形式でチャットや共有ドキュメントへ短く記録してから、答え合わせと整理に進みます。
...
16点以上なら、シニアのレビュー下で局所的な設計判断に参加できる状態とみなします。
